...
// The component now accepts 'logoSrc' as a prop
const Navbar = ({ logoSrc, brandName, navLinks, activeSection, scrollToSection }) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            {/* Updated this section to include the logo */}
            <div onClick={() => scrollToSection('home')} className="cursor-pointer flex items-center flex-shrink-0">
              {/* Logo Image */}
              <img 
                className="h-10 w-10 mr-3 rounded-full object-cover shadow-amber-400/30 shadow-md" 
                src={logoSrc} 
                alt={`${brandName} Logo`} 
              />
              {/* Brand Name */}
              <h1 className="text-xl sm:text-2xl font-bold text-white transition-colors duration-300 hover:text-amber-400">
                {brandName}
              </h1>
            </div>
          </div>
